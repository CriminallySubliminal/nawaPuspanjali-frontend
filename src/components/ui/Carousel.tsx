...
function CarouselItem({ item, index, itemWidth, round, trackItemOffset, x, transition }: CarouselItemProps) {
    ...
    const content = (
        <motion.div
            ...
            style={{
                ...
                ...(item.color && !round && { borderTop: `4px solid ${item.color}` })
            }}
    ...
    if (item.href) {
        return (
            <Link to={item.href} className="carousel-item-link" style={{ width: itemWidth }}>
                {content}
            </Link>
        );
    }
...
export default function Carousel({
    ...
    useEffect(() => {
        ...
        const timer = setInterval(() => {
            setPosition(prev => {
                const next = prev + 1;
                if (next >= itemsForRender.length) return loop ? 1 : 0;
                return next;
            });
        }, autoplayDelay);
